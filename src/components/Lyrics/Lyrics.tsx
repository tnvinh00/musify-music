      ...
      <div className="md:px-4 flex items-center w-full text-center">
        <Lrc
          ...
          lineRenderer={({ index, active, line }) => (
            <p
              key={index}
              className={`text-lg md:text-2xl transition-all duration-500 ${active ? 'text-yellow-400 dark:text-yellow-300 text-shadow-yellow text-xl md:text-3xl font-medium my-6' : 'my-4'}`}
            >
              {line.content}
            </p>
          )}
